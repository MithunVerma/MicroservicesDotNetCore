...
namespace Catalog.Infrastructure.Repositories
{
    public class ProductRepository : IProductRepository, IBrandRepository, ITypesRepository
    {
        ...
        public async Task<Pagination<Product>> GetProducts(CatalagSpecParams catalagSpecParams)
        {
            ...
            var data = await DataFilter(catalagSpecParams, filter);
        ...
        private async Task<IReadOnlyList<Product>> DataFilter(CatalagSpecParams catalagSpecParams, FilterDefinition<Product> filter)
        {
            var sortDefn = Builders<Product>.Sort.Ascending("Name");
            if (!string.IsNullOrEmpty(catalagSpecParams.Sort))
            {
                switch (catalagSpecParams.Sort)
                {
                    case "priceAsc":
            ...
            return await _catalogContext
                         ...
                         .Find(filter)
                         .Sort(sortDefn)
                         .Skip(catalagSpecParams.PageSize * (catalagSpecParams.PageIndex - 1))
                         .Limit(catalagSpecParams.PageSize)
                         .ToListAsync();
        }
